...
export class GetTopSongsService
  implements
    IApplicationService<
    ...
    >
{
  ...
  async execute(
    param: GetTopSongsEntryApplicationDto,
  ): Promise<Result<GetTopSongsResponseApplicationDto>> {
    const SongsResult = await this.SongRepository.findTopSongs();
    if (!SongsResult.IsSuccess) {
      return Result.fail<GetTopSongsResponseApplicationDto>(
        null,
    ...
    for (let i = 0; i < SongsResult.Data.length; i++) {
      ...
      const imageResult = await (
        await this.getBufferImage.getFile(song.Cover.Path)
      ).Data;
      const songObject = {
        songId: song.Id.Id,
        name: song.Name.Name,
      ...
      const artists = await this.artistRepository.findArtistsBySongId(song.Id);
    ...
    return Result.success<GetTopSongsResponseApplicationDto>(
      response,
      SongsResult.statusCode,
    );
  }
