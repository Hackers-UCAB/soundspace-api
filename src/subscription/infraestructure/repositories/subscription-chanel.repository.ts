  ...
  implements ISubscriptionChanelRepository
{
  ...
  async findSubscriptionChanelById(
    ...
    try {
      ...
      if(error){
        return Result.fail(null, 500, 'Error al buscar o mapear el canal', error);
      }
      if(!response){
        return Result.fail(null, 404, 'No existe el canal de subscripcion solicitado', new Error('No existe el canal de subscripcion solicitado'));
      }
